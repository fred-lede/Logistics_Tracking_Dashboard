...
export async function POST(request: Request) {
  try {
    ...
    const llmProvider = await resolveProvider(provider, apiKey, baseUrl, model, providerLabel, { compatMode })
    if (!llmProvider) {
    ...
    console.error('[api/llm/test-notification] Unhandled error:', err)
    return NextResponse.json(
